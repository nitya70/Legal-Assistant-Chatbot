...
# Load vector DBs for different legal domains
vector_dbs = {
    "family": FAISS.load_local("vectorstores/family", embeddings, allow_dangerous_deserialization=True),
    "crime": FAISS.load_local("vectorstores/crime", embeddings, allow_dangerous_deserialization=True),
    "land": FAISS.load_local("vectorstores/land", embeddings, allow_dangerous_deserialization=True),
}
# Initialize retriever for each domain
retrievers = {domain: db.as_retriever(search_type="similarity", search_kwargs={"k": 4}) for domain, db in vector_dbs.items()}
# Initialize the retriever based on the categorized query
def db_retriever(query: str):
    category = categorize_query(query)
    return retrievers.get(category, retrievers["family"])  # Default to family if category not found
# Load the vector store for the default category
#db = FAISS.load_local("vectorstores/family", embeddings, allow_dangerous_deserialization=True)
#db = FAISS.load_local("vectorstores/crime", embeddings, allow_dangerous_deserialization=True)
#db = FAISS.load_local("vectorstores/land", embeddings, allow_dangerous_deserialization=True)
#db = FAISS.load_local("law_vector_db", embeddings, allow_dangerous_deserialization=True)
#db_retriever = db.as_retriever(search_type="similarity", search_kwargs={"k": 4})

def categorize_query(query: str) -> str:
    q = query.lower()
    if any(k in q for k in ["marriage", "divorce", "child", "custody", "alimony", "maintenance", "dowry", "family"]):
        return "family"
    elif any(k in q for k in ["murder", "theft", "assault", "rape", "police", "arrest", "criminal", "crime"]):
        return "crime"
    elif any(k in q for k in ["land", "property", "ownership", "dispute", "boundary", "registration", "plot"]):
        return "land"
    else:
        return "family"  # Default fallback
# Initialize the vector store based on the categorized query
...
TOGETHER_AI_API= '1e45b83a953c8b03e478917aa7cdbb4f55918214c9279de5abf258229a74f90d'
llm = Together(
...
# ✅ HANDLE: Only if the user sent something
if input_prompt:
